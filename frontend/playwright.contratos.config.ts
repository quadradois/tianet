...
const frontendPort = 3105;
const backendPort = 3205;
const baseURL = `http://127.0.0.1:${frontendPort}`;

export default defineConfig({
  testDir: "./tests/contratos-e2e",
  fullyParallel: false,
  forbidOnly: true,
  retries: process.env.CI ? 1 : 0,
  workers: 1,
  outputDir: "test-results/contratos",
  reporter: [["line"], ["html", { open: "never", outputFolder: "playwright-report/contratos" }]],
  ...
  webServer: [
    { command: `node tests/contratos-e2e/backend-fixture.mjs --port ${backendPort}`, reuseExistingServer: false, timeout: 30_000, url: `http://127.0.0.1:${backendPort}/health` },
    {
      ...
      env: {
        ...
        FRONTEND_SESSION_KEY_ID: "contratos-current",
      },
