...
export function IamLoadingState() {
  return (
    <Card className="min-w-0">
      <CardHeader>
        <CardTitle>IAM permitido</CardTitle>
        <CardDescription>loading catalogo, Perfis e Usuario conhecido...</CardDescription>
      </CardHeader>
...
export function IamProblemState({ problem }: Readonly<{ problem: IamProblem }>) {
  ...
  return (
    <Alert role="alert" variant="danger">
      <AlertTitle>{neutral404 ? "IAM nao encontrado ou indisponivel (404)" : `Erro ${problem.status}`}</AlertTitle>
      <AlertDescription>
        {neutral404 ? "Recurso IAM nao encontrado ou indisponivel." : problem.mensagem} Correlation ID: {problem.correlationId}
      </AlertDescription>
    </Alert>
  );
}

function DeniedState({ children = "Sem permissao IAM exata." }: Readonly<{ children?: ReactNode }>) {
...
function PerfilList({ perfis }: Readonly<{ perfis: readonly Perfil[] }>) {
  if (perfis.length === 0) return <p role="status">empty: nenhum Perfil retornado pelo backend.</p>;
  return (
    <div aria-label="Perfis IAM com overflow" className="overflow-x-auto rounded-md border" role="region" tabIndex={0}>
      <table className="w-full min-w-[54rem] text-left text-sm">
        <caption className="sr-only">Perfis de acesso retornados pelo backend</caption>
        ...
        <tbody>
          {perfis.map((perfil) => (
            <tr className="border-t" key={perfil.id}>
              <td className="break-words p-2 font-semibold">{perfil.nome}<span className="block text-xs text-muted-foreground">{perfil.id}</span></td>
...
function CatalogoView({ catalogo }: Readonly<{ catalogo: PermissoesCatalogo }>) {
  return (
    <Card className="min-w-0">
      <CardHeader><CardTitle>Catalogo canonico</CardTitle><CardDescription>Versao {catalogo.versao}. Codigos vêm de /iam/permissoes.</CardDescription></CardHeader>
      <CardContent>
...
function PerfilDetail({ perfil }: Readonly<{ perfil: Perfil | null }>) {
  if (!perfil) return <p role="status">Defina perfil_id na URL para consultar um Perfil especifico.</p>;
  return (
    <Card className="min-w-0">
      <CardHeader><CardTitle>{perfil.nome}</CardTitle><CardDescription>Perfil retornado pelo backend para Tenant corrente.</CardDescription></CardHeader>
...
function UsuarioPermissoes({ value }: Readonly<{ value: PermissoesEfetivas | null }>) {
  if (!value) return <p role="status">Informe usuario_id para consultar permissoes efetivas de um Usuario conhecido.</p>;
  return (
    <Card>
      <CardHeader><CardTitle>Permissoes efetivas</CardTitle><CardDescription>Usuario conhecido {value.usuario_id}; sem listagem de Usuarios.</CardDescription></CardHeader>
      <CardContent className="grid gap-2 text-sm">
  ...
  return (
    <form className="grid gap-3 rounded-lg border bg-card p-4 md:grid-cols-[1fr_1fr_auto]" method="get">
      <div className="grid gap-2">
        <Label htmlFor="iam-filter-perfil">perfil_id</Label>
        <Input defaultValue={filters.perfilId ?? ""} id="iam-filter-perfil" name="perfil_id" placeholder="UUID do Perfil" />
      </div>
      <div className="grid gap-2">
        <Label htmlFor="iam-filter-usuario">usuario_id conhecido</Label>
        <Input defaultValue={filters.usuarioId ?? ""} id="iam-filter-usuario" name="usuario_id" placeholder="UUID do Usuario conhecido" />
      </div>
      <Button className="self-end" type="submit">Consultar IAM</Button>
    </form>
  );
}
...
export async function IamAdmin({ actions, catalogo, filters, perfil, perfis, permissions, recoveryHref, usuarioPermissoes }: IamAdminProps) {
  ...
  return (
    <div className="grid min-w-0 gap-6">
      <header>
        <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">IAM permitido</p>
        <h1 className="text-balance text-3xl font-bold tracking-tight">Perfis, catalogo e atribuicoes</h1>
        <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
          Escopo limitado aos contratos existentes: Perfis, catalogo canonico e Usuario conhecido. Nao ha lista de Usuarios, credenciais ou gestao integral.
        </p>
      </header>
      <FilterForm filters={filters} />
      {!canRead ? <DeniedState>Sem permissao perfil.ler para consultar IAM.</DeniedState> : null}
      <div className="grid min-w-0 gap-5 xl:grid-cols-2">
        <Card className="min-w-0">
          <CardHeader><CardTitle>Perfis</CardTitle><CardDescription>Lista oficial retornada por /iam/perfis.</CardDescription></CardHeader>
      ...
      {canManage ? <IamActions {...actions} /> : <DeniedState>Sem permissao perfil.gerir para comandos IAM.</DeniedState>}
    </div>
  );
}
