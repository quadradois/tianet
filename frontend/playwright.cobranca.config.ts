...
const frontendPort = 3107;
const backendPort = 3207;
const baseURL = `http://127.0.0.1:${frontendPort}`;

export default defineConfig({
  forbidOnly: true,
  fullyParallel: false,
  outputDir: "test-results/cobranca",
  projects: [
  ...
  reporter: [["line"], ["html", { open: "never", outputFolder: "playwright-report/cobranca" }]],
  retries: process.env.CI ? 1 : 0,
  testDir: "./tests/cobranca-e2e",
  use: { baseURL, screenshot: "only-on-failure", trace: "retain-on-failure" },
  webServer: [
    { command: `node tests/cobranca-e2e/backend-fixture.mjs --port ${backendPort}`, reuseExistingServer: false, timeout: 30_000, url: `http://127.0.0.1:${backendPort}/health` },
    {
      ...
      env: {
        ...
        FRONTEND_SESSION_KEY_ID: "cobranca-current",
      },
