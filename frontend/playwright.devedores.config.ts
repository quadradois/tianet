...
const backendPort = 3203;
const baseURL = `http://127.0.0.1:${frontendPort}`;

export default defineConfig({
  testDir: "./tests/devedores-e2e",
  fullyParallel: false,
  forbidOnly: true,
  retries: process.env.CI ? 1 : 0,
  workers: 1,
  outputDir: "test-results/devedores",
  reporter: [["line"], ["html", { open: "never", outputFolder: "playwright-report/devedores" }]],
  ...
  webServer: [
    { command: `node tests/devedores-e2e/backend-fixture.mjs --port ${backendPort}`, reuseExistingServer: false, timeout: 30_000, url: `http://127.0.0.1:${backendPort}/health` },
    {
      ...
      env: {
        ...
        FRONTEND_SESSION_KEY_ID: "devedores-current",
      },
