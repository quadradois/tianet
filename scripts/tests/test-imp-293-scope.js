...
const manifestPath = path.join(root, 'docs/audits/evidence/frontend-mvp-imp-293-protected-baseline.json');
const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
const divergences = [];
const sha256 = (absolute) => crypto.createHash('sha256').update(fs.readFileSync(absolute)).digest('hex');

assert.strictEqual(manifest.baselineCount, 215, 'baseline IMP-293 deve conter 215 paths');
assert.strictEqual(manifest.mutableBaselinePaths.length, 11, 'allowlist mutavel IMP-293 deve ter 11 paths');
assert.strictEqual(manifest.protectedBaselineCount, 204, 'IMP-293 deve proteger 204 paths');
assert.strictEqual(manifest.allowedNewPaths.length, 22, 'allowlist nova IMP-293 deve ter 22 paths');
assert.strictEqual(manifest.expectedFinalCount, 237, 'inventario final IMP-293 deve ter 237 paths');
...
assert.strictEqual(sha256(predecessorPath), manifest.predecessor.sha256, 'manifesto verificado do IMP-292 foi reescrito');
...
], 'IMP-293 nao pode criar Route Handler publico de jornada');
...
for (const relative of actualPaths) {
  if (!expectedPaths.has(relative)) divergences.push(`${relative}: caminho extra fora da allowlist do IMP-293`);
}

assert.strictEqual(protectedCount, manifest.protectedBaselineCount, 'contagem protegida IMP-293');
assert.strictEqual(actualPaths.size, manifest.expectedFinalCount, 'contagem do delta final IMP-293');
assert.strictEqual(divergences.length, 0, `escopo protegido do IMP-293 divergiu:\n${divergences.join('\n')}`);
console.log(`IMP-293 scope: predecessor verificado, ${protectedCount} arquivos protegidos e delta exato commitado/worktree (${actualPaths.size} paths), 0 divergencia.`);
