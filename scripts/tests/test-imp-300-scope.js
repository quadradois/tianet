...
const manifestPath = path.join(root, 'docs/audits/evidence/frontend-mvp-imp-300-protected-baseline.json');
...
assert.strictEqual(manifest.baselineCount, 368, 'baseline IMP-300 deve conter 368 paths');
assert.strictEqual(manifest.mutableBaselinePaths.length, 9, 'allowlist mutavel IMP-300 deve ter 9 paths');
assert.strictEqual(manifest.protectedBaselineCount, 359, 'IMP-300 deve proteger 359 paths');
assert.strictEqual(manifest.allowedNewPaths.length, 22, 'allowlist nova IMP-300 deve ter 22 paths');
assert.strictEqual(manifest.expectedFinalCount, 390, 'inventario final IMP-300 deve ter 390 paths');
...
assert.strictEqual(sha256(predecessorPath), manifest.predecessor.sha256, 'manifesto verificado do IMP-299 foi reescrito');
...
], 'IMP-300 nao pode criar Route Handler publico de Automacao');
...
for (const relative of actualPaths) {
  if (!expectedPaths.has(relative)) divergences.push(`${relative}: caminho extra fora da allowlist IMP-300`);
}

assert.strictEqual(protectedCount, manifest.protectedBaselineCount, 'contagem protegida IMP-300');
assert.strictEqual(actualPaths.size, manifest.expectedFinalCount, 'contagem do delta final IMP-300');
assert.strictEqual(divergences.length, 0, `escopo protegido IMP-300 divergiu:\n${divergences.join('\n')}`);
console.log(`IMP-300 scope: predecessor verificado, ${protectedCount} arquivos protegidos e delta exato commitado/worktree (${actualPaths.size} paths), 0 divergencia.`);
