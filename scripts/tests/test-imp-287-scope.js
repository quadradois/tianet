...
assert.strictEqual(
  ...
  'manifesto verificado do IMP-286 foi reescrito',
);
...
const committedDelta = childProcess.execFileSync(
  'git',
  ['diff', '--name-only', `${manifest.head}...HEAD`],
  { cwd: root, encoding: 'utf8' },
);
...
for (const relative of actualPaths) {
  if (!expectedPaths.has(relative)) divergences.push(`${relative}: caminho extra fora da allowlist do IMP-287`);
}

assert.strictEqual(
  divergences.length,
  0,
  `escopo protegido do IMP-287 divergiu:\n${divergences.join('\n')}`,
);
console.log(
  `IMP-287 scope: predecessor verificado, ${protectedCount} arquivos protegidos e delta exato commitado/worktree, 0 divergencia.`,
);
