...
const manifestPath = path.join(root, 'docs/audits/evidence/frontend-mvp-bff-error-sanitization-protected-baseline.json');
const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
const divergences = [];
const sha256 = (absolute) => crypto.createHash('sha256').update(fs.readFileSync(absolute)).digest('hex');

assert.strictEqual(manifest.baselineCount, 301, 'baseline sanitizacao BFF deve conter 301 paths');
assert.strictEqual(manifest.mutableBaselinePaths.length, 12, 'allowlist mutavel sanitizacao BFF deve ter 12 paths');
assert.strictEqual(manifest.protectedBaselineCount, 289, 'sanitizacao BFF deve proteger 289 paths');
assert.strictEqual(manifest.allowedNewPaths.length, 3, 'allowlist nova sanitizacao BFF deve ter 3 paths');
assert.strictEqual(manifest.expectedFinalCount, 304, 'inventario final sanitizacao BFF deve ter 304 paths');
...
assert.strictEqual(sha256(predecessorPath), manifest.predecessor.sha256, 'manifesto verificado do IMP-296 foi reescrito');
...
], 'sanitizacao BFF nao pode criar Route Handler publico de jornada');
...
for (const relative of actualPaths) {
  if (!expectedPaths.has(relative)) divergences.push(`${relative}: caminho extra fora da allowlist de sanitizacao BFF`);
}

assert.strictEqual(protectedCount, manifest.protectedBaselineCount, 'contagem protegida sanitizacao BFF');
assert.strictEqual(actualPaths.size, manifest.expectedFinalCount, 'contagem do delta final sanitizacao BFF');
assert.strictEqual(divergences.length, 0, `escopo protegido da sanitizacao BFF divergiu:\n${divergences.join('\n')}`);
console.log(`BFF error sanitization scope: predecessor verificado, ${protectedCount} arquivos protegidos e delta exato commitado/worktree (${actualPaths.size} paths), 0 divergencia.`);
