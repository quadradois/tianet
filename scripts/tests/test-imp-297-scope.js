...
const manifestPath = path.join(root, 'docs/audits/evidence/frontend-mvp-imp-297-protected-baseline.json');
const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
const divergences = [];
const sha256 = (absolute) => crypto.createHash('sha256').update(fs.readFileSync(absolute)).digest('hex');

assert.strictEqual(manifest.baselineCount, 304, 'baseline IMP-297 deve conter 304 paths');
assert.strictEqual(manifest.mutableBaselinePaths.length, 10, 'allowlist mutavel IMP-297 deve ter 10 paths');
assert.strictEqual(manifest.protectedBaselineCount, 294, 'IMP-297 deve proteger 294 paths');
assert.strictEqual(manifest.allowedNewPaths.length, 20, 'allowlist nova IMP-297 deve ter 20 paths');
assert.strictEqual(manifest.expectedFinalCount, 324, 'inventario final IMP-297 deve ter 324 paths');
...
assert.strictEqual(sha256(predecessorPath), manifest.predecessor.sha256, 'manifesto verificado da sanitizacao BFF foi reescrito');
...
], 'IMP-297 nao pode criar Route Handler publico de Relatorios');
...
for (const relative of actualPaths) {
  if (!expectedPaths.has(relative)) divergences.push(`${relative}: caminho extra fora da allowlist IMP-297`);
}

assert.strictEqual(protectedCount, manifest.protectedBaselineCount, 'contagem protegida IMP-297');
assert.strictEqual(actualPaths.size, manifest.expectedFinalCount, 'contagem do delta final IMP-297');
assert.strictEqual(divergences.length, 0, `escopo protegido IMP-297 divergiu:\n${divergences.join('\n')}`);
console.log(`IMP-297 scope: predecessor verificado, ${protectedCount} arquivos protegidos e delta exato commitado/worktree (${actualPaths.size} paths), 0 divergencia.`);
