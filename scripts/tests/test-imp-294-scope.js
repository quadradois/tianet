...
const manifestPath = path.join(root, 'docs/audits/evidence/frontend-mvp-imp-294-protected-baseline.json');
const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
const divergences = [];
const sha256 = (absolute) => crypto.createHash('sha256').update(fs.readFileSync(absolute)).digest('hex');

assert.strictEqual(manifest.baselineCount, 237, 'baseline IMP-294 deve conter 237 paths');
assert.strictEqual(manifest.mutableBaselinePaths.length, 11, 'allowlist mutavel IMP-294 deve ter 11 paths');
assert.strictEqual(manifest.protectedBaselineCount, 226, 'IMP-294 deve proteger 226 paths');
assert.strictEqual(manifest.allowedNewPaths.length, 22, 'allowlist nova IMP-294 deve ter 22 paths');
assert.strictEqual(manifest.expectedFinalCount, 259, 'inventario final IMP-294 deve ter 259 paths');
...
assert.strictEqual(sha256(predecessorPath), manifest.predecessor.sha256, 'manifesto verificado do IMP-293 foi reescrito');
...
], 'IMP-294 nao pode criar Route Handler publico de jornada');
...
for (const relative of actualPaths) {
  if (!expectedPaths.has(relative)) divergences.push(`${relative}: caminho extra fora da allowlist do IMP-294`);
}

assert.strictEqual(protectedCount, manifest.protectedBaselineCount, 'contagem protegida IMP-294');
assert.strictEqual(actualPaths.size, manifest.expectedFinalCount, 'contagem do delta final IMP-294');
assert.strictEqual(divergences.length, 0, `escopo protegido do IMP-294 divergiu:\n${divergences.join('\n')}`);
console.log(`IMP-294 scope: predecessor verificado, ${protectedCount} arquivos protegidos e delta exato commitado/worktree (${actualPaths.size} paths), 0 divergencia.`);
