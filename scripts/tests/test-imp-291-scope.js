...
const manifestPath = path.join(root, 'docs/audits/evidence/frontend-mvp-imp-291-protected-baseline.json');
const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
const divergences = [];
const sha256 = (absolute) => crypto.createHash('sha256').update(fs.readFileSync(absolute)).digest('hex');

assert.strictEqual(manifest.baselineCount, 169, 'baseline IMP-291 deve conter 169 paths');
assert.strictEqual(manifest.mutableBaselinePaths.length, 15, 'allowlist mutavel IMP-291 deve ter 15 paths');
assert.strictEqual(manifest.protectedBaselineCount, 154, 'IMP-291 deve proteger 154 paths');
assert.strictEqual(manifest.allowedNewPaths.length, 23, 'allowlist nova IMP-291 deve ter 23 paths');
assert.strictEqual(manifest.expectedFinalCount, 192, 'inventario final IMP-291 deve ter 192 paths');
...
assert.strictEqual(sha256(predecessorPath), manifest.predecessor.sha256, 'manifesto verificado do IMP-290 foi reescrito');
...
], 'IMP-291 nao pode criar Route Handler publico de jornada');
...
for (const relative of actualPaths) {
  if (!expectedPaths.has(relative)) divergences.push(`${relative}: caminho extra fora da allowlist do IMP-291`);
}

assert.strictEqual(protectedCount, manifest.protectedBaselineCount, 'contagem protegida IMP-291');
assert.strictEqual(actualPaths.size, manifest.expectedFinalCount, 'contagem do delta final IMP-291');
assert.strictEqual(divergences.length, 0, `escopo protegido do IMP-291 divergiu:\n${divergences.join('\n')}`);
console.log(`IMP-291 scope: predecessor verificado, ${protectedCount} arquivos protegidos e delta exato commitado/worktree (${actualPaths.size} paths), 0 divergencia.`);
