...
const manifestPath = path.join(root, 'docs/audits/evidence/frontend-mvp-imp-298-protected-baseline.json');
const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
const divergences = [];
const sha256 = (absolute) => crypto.createHash('sha256').update(fs.readFileSync(absolute)).digest('hex');

assert.strictEqual(manifest.baselineCount, 324, 'baseline IMP-298 deve conter 324 paths');
assert.strictEqual(manifest.mutableBaselinePaths.length, 9, 'allowlist mutavel IMP-298 deve ter 9 paths');
assert.strictEqual(manifest.protectedBaselineCount, 315, 'IMP-298 deve proteger 315 paths');
assert.strictEqual(manifest.allowedNewPaths.length, 22, 'allowlist nova IMP-298 deve ter 22 paths');
assert.strictEqual(manifest.expectedFinalCount, 346, 'inventario final IMP-298 deve ter 346 paths');
...
assert.strictEqual(sha256(predecessorPath), manifest.predecessor.sha256, 'manifesto verificado do IMP-297 foi reescrito');
...
], 'IMP-298 nao pode criar Route Handler publico de Configuracoes');
...
for (const relative of actualPaths) {
  if (!expectedPaths.has(relative)) divergences.push(`${relative}: caminho extra fora da allowlist IMP-298`);
}

assert.strictEqual(protectedCount, manifest.protectedBaselineCount, 'contagem protegida IMP-298');
assert.strictEqual(actualPaths.size, manifest.expectedFinalCount, 'contagem do delta final IMP-298');
assert.strictEqual(divergences.length, 0, `escopo protegido IMP-298 divergiu:\n${divergences.join('\n')}`);
console.log(`IMP-298 scope: predecessor verificado, ${protectedCount} arquivos protegidos e delta exato commitado/worktree (${actualPaths.size} paths), 0 divergencia.`);
