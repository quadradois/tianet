...
const manifestPath = path.join(root, 'docs/audits/evidence/frontend-mvp-imp-299-protected-baseline.json');
const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
const divergences = [];
const sha256 = (absolute) => crypto.createHash('sha256').update(fs.readFileSync(absolute)).digest('hex');

assert.strictEqual(manifest.baselineCount, 346, 'baseline IMP-299 deve conter 346 paths');
assert.strictEqual(manifest.mutableBaselinePaths.length, 9, 'allowlist mutavel IMP-299 deve ter 9 paths');
assert.strictEqual(manifest.protectedBaselineCount, 337, 'IMP-299 deve proteger 337 paths');
assert.strictEqual(manifest.allowedNewPaths.length, 22, 'allowlist nova IMP-299 deve ter 22 paths');
assert.strictEqual(manifest.expectedFinalCount, 368, 'inventario final IMP-299 deve ter 368 paths');
...
assert.strictEqual(sha256(predecessorPath), manifest.predecessor.sha256, 'manifesto verificado do IMP-298 foi reescrito');
...
], 'IMP-299 nao pode criar Route Handler publico de IAM');
...
for (const relative of actualPaths) {
  if (!expectedPaths.has(relative)) divergences.push(`${relative}: caminho extra fora da allowlist IMP-299`);
}

assert.strictEqual(protectedCount, manifest.protectedBaselineCount, 'contagem protegida IMP-299');
assert.strictEqual(actualPaths.size, manifest.expectedFinalCount, 'contagem do delta final IMP-299');
assert.strictEqual(divergences.length, 0, `escopo protegido IMP-299 divergiu:\n${divergences.join('\n')}`);
console.log(`IMP-299 scope: predecessor verificado, ${protectedCount} arquivos protegidos e delta exato commitado/worktree (${actualPaths.size} paths), 0 divergencia.`);
