...
const contracts = {
  ...
  hardening(source) {
    ...
    assert.strictEqual(operations, 105, 'snapshot deve conter 105 operacoes');
    assert.strictEqual(Object.keys(snapshot.components.schemas).length, 131, 'snapshot deve conter 131 schemas');
  ...
  harness(source = readHarness()) {
    ...
    assertText(source.contractSmoke, '105', 'contrato deve validar 105 operacoes');
    assertText(source.contractSmoke, '131', 'contrato deve validar 131 schemas');
  ...
  openapiClient(source = readOpenapiClient()) {
    ...
    for (const text of ['toHaveLength(62)', 'required).toBe(true)', 'minLength: 1, maxLength: 255', 'toBe(105)', 'toHaveLength(131)']) {
      assertText(source.contractTest, text, `teste contratual cobre ${text}`);
    }
  ...
  finalReadiness(source = readFinalReadiness()) {
    ...
    assert.strictEqual(operationCount, 105, 'OpenAPI reflete a remocao do IMP-351');
    assert.strictEqual(schemaCount, 131, 'OpenAPI reflete a remocao do IMP-351');
    const openapiSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT, FINAL_READINESS_FILES.openapi))).digest('hex');
    assert.strictEqual(openapiSha, 'e87bdad9b000959dea7809878cdd69c6cfcdfca2a2dc5fa8e9cc4cc7bd5e16e6', 'OpenAPI corresponde ao snapshot governado vigente');
  },
};
