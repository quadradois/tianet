...
const contracts = {
  ...
  hardening(source) {
    ...
    // ADR-020, que publicou as quatro rotas administrativas OpenAI; 116 com a
    // S3, que publicou GET /platform/agent/inbox (somente leitura); 119 com o
    // IMP-389 (GET /credit/emprestimos/{id}/alocacao-prevista);
    // IMP-353, que publicou GET/PUT /platform/whatsapp/avisos.
    assert.strictEqual(operations, 124, 'snapshot deve conter 124 operacoes');
    assert.strictEqual(Object.keys(snapshot.components.schemas).length, 153, 'snapshot deve conter 153 schemas');
  ...
  harness(source = readHarness()) {
    ...
    assertText(source.contractSmoke, '124', 'contrato deve validar 124 operacoes');
    assertText(source.contractSmoke, '153', 'contrato deve validar 153 schemas');
  ...
  foundation(source = readFoundation()) {
    ...
    assert.deepStrictEqual(
      ...
      ['components/configuracoes-financeiras/configuracoes-actions.client.tsx', 'components/foundation/destructive-dialog-demo.tsx', 'components/openai/openai.client.tsx', 'components/pagamentos/mercadopago.client.tsx', 'components/ui/dialog.tsx', 'components/ui/sheet.tsx', 'components/whatsapp/numero-avisos.client.tsx', 'components/whatsapp/whatsapp.client.tsx'],
  ...
  openapiClient(source = readOpenapiClient()) {
    ...
    assertText(
      source.generated,
      'e3b75133a010c657638e84f3b16adbd830e6dbbedef9ce40c33b7b0a013c80f6',
    ...
    for (const text of ['toBe(124)', 'toHaveLength(153)', 'toHaveLength(69)', 'required).toBe(true)', 'minLength: 1, maxLength: 255']) {
      assertText(source.contractTest, text, `teste contratual cobre ${text}`);
    }
  ...
  finalReadiness(source = readFinalReadiness()) {
    ...
    // rotas administrativas OpenAI; 116 com a S3 (GET /platform/agent/inbox);
    // 119 com o IMP-353 (GET/PUT /platform/whatsapp/avisos).
    assert.strictEqual(operationCount, 124, 'OpenAPI reflete as rotas do IMP-353');
    assert.strictEqual(schemaCount, 153, 'OpenAPI reflete as rotas do IMP-353');
    const openapiSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT, FINAL_READINESS_FILES.openapi))).digest('hex');
    assert.strictEqual(openapiSha, 'e3b75133a010c657638e84f3b16adbd830e6dbbedef9ce40c33b7b0a013c80f6', 'OpenAPI corresponde ao snapshot governado vigente');
  },
};
