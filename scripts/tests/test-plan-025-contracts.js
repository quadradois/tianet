...
const contracts = {
  ...
  hardening(source) {
    ...
    // S3, que publicou GET /platform/agent/inbox (somente leitura).
    assert.strictEqual(operations, 116, 'snapshot deve conter 116 operacoes');
    assert.strictEqual(Object.keys(snapshot.components.schemas).length, 148, 'snapshot deve conter 148 schemas');
  ...
  harness(source = readHarness()) {
    ...
    assertText(source.contractSmoke, '116', 'contrato deve validar 116 operacoes');
    assertText(source.contractSmoke, '148', 'contrato deve validar 148 schemas');
  ...
  openapiClient(source = readOpenapiClient()) {
    ...
    for (const text of ['toBe(116)', 'toHaveLength(148)', 'toHaveLength(64)', 'required).toBe(true)', 'minLength: 1, maxLength: 255']) {
      assertText(source.contractTest, text, `teste contratual cobre ${text}`);
    }
  ...
  finalReadiness(source = readFinalReadiness()) {
    ...
    assert.strictEqual(operationCount, 116, 'OpenAPI reflete a rota da S3');
    assert.strictEqual(schemaCount, 148, 'OpenAPI reflete a rota da S3');
    const openapiSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT, FINAL_READINESS_FILES.openapi))).digest('hex');
    assert.strictEqual(openapiSha, '18a29b265e9f0b785145458cf56a6d90a83a4b41a8f63cf9c576237dfdb7cc4a', 'OpenAPI corresponde ao snapshot governado vigente');
  },
};
