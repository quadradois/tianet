...
const contracts = {
  ...
  openapiClient(source = readOpenapiClient()) {
    ...
    for (const text of ['toHaveLength(63)', 'required).toBe(true)', 'minLength: 1, maxLength: 255', 'toBe(106)', 'toHaveLength(133)']) {
      assertText(source.contractTest, text, `teste contratual cobre ${text}`);
    }
  ...
  finalReadiness(source = readFinalReadiness()) {
    ...
    assert.strictEqual(operationCount, 106, 'OpenAPI reflete o endpoint do IMP-355');
    assert.strictEqual(schemaCount, 133, 'OpenAPI reflete o endpoint do IMP-355');
    const openapiSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT, FINAL_READINESS_FILES.openapi))).digest('hex');
    assert.strictEqual(openapiSha, '63f7331c1b9aee898c1c6426aa9e1f64effe59a5536e022ccc29311685f21957', 'OpenAPI corresponde ao snapshot governado vigente');
  },
};
