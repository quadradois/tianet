...
const contracts = {
  ...
  hardening(source) {
    ...
    // S3, que publicou GET /platform/agent/inbox (somente leitura); 118 com o
    // IMP-353, que publicou GET/PUT /platform/whatsapp/avisos.
    assert.strictEqual(operations, 118, 'snapshot deve conter 118 operacoes');
    assert.strictEqual(Object.keys(snapshot.components.schemas).length, 150, 'snapshot deve conter 150 schemas');
  ...
  harness(source = readHarness()) {
    ...
    assertText(source.contractSmoke, '118', 'contrato deve validar 118 operacoes');
    assertText(source.contractSmoke, '150', 'contrato deve validar 150 schemas');
  ...
  openapiClient(source = readOpenapiClient()) {
    ...
    for (const text of ['toBe(118)', 'toHaveLength(150)', 'toHaveLength(65)', 'required).toBe(true)', 'minLength: 1, maxLength: 255']) {
      assertText(source.contractTest, text, `teste contratual cobre ${text}`);
    }
  ...
  finalReadiness(source = readFinalReadiness()) {
    ...
    // 118 com o IMP-353 (GET/PUT /platform/whatsapp/avisos).
    assert.strictEqual(operationCount, 118, 'OpenAPI reflete as rotas do IMP-353');
    assert.strictEqual(schemaCount, 150, 'OpenAPI reflete as rotas do IMP-353');
    const openapiSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT, FINAL_READINESS_FILES.openapi))).digest('hex');
    assert.strictEqual(openapiSha, 'c0bfa8b85e1651a12a86f836e64f01e3f5ba9a075768e1911ffaeac42c435b3e', 'OpenAPI corresponde ao snapshot governado vigente');
  },
};
