...
const contracts = {
  ...
  hardening(source) {
    ...
    assert.strictEqual(operations, 128, 'snapshot deve conter 128 operacoes');
    assert.strictEqual(Object.keys(snapshot.components.schemas).length, 157, 'snapshot deve conter 157 schemas');
  ...
  harness(source = readHarness()) {
    ...
    assertText(source.contractSmoke, '128', 'contrato deve validar 128 operacoes');
    assertText(source.contractSmoke, '157', 'contrato deve validar 157 schemas');
  ...
  foundation(source = readFoundation()) {
    ...
    assert.deepStrictEqual(
      ...
      ['components/configuracoes-financeiras/configuracoes-actions.client.tsx', 'components/foundation/destructive-dialog-demo.tsx', 'components/openai/openai.client.tsx', 'components/pagamentos/chave-pix.client.tsx', 'components/pagamentos/mercadopago.client.tsx', 'components/ui/dialog.tsx', 'components/ui/sheet.tsx', 'components/whatsapp/numero-avisos.client.tsx', 'components/whatsapp/whatsapp.client.tsx'],
  ...
  openapiClient(source = readOpenapiClient()) {
    ...
    for (const text of ['toBe(128)', 'toHaveLength(157)', 'toHaveLength(71)', 'required).toBe(true)', 'minLength: 1, maxLength: 255']) {
      assertText(source.contractTest, text, `teste contratual cobre ${text}`);
    }
  ...
  finalReadiness(source = readFinalReadiness()) {
    ...
    assert.strictEqual(operationCount, 128, 'OpenAPI reflete as rotas do IMP-353');
    assert.strictEqual(schemaCount, 157, 'OpenAPI reflete as rotas do IMP-353');
    const openapiSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT, FINAL_READINESS_FILES.openapi))).digest('hex');
    assert.strictEqual(openapiSha, 'f433314df34f67bcfa4766606f64d2ee1c8aed902c5243da6a73860077bff52a', 'OpenAPI corresponde ao snapshot governado vigente');
  },
};
