...
const contracts = {
  ...
  openapiClient(source = readOpenapiClient()) {
    ...
    for (const text of ['toHaveLength(63)', 'required).toBe(true)', 'minLength: 1, maxLength: 255', 'toBe(111)', 'toHaveLength(137)']) {
      assertText(source.contractTest, text, `teste contratual cobre ${text}`);
    }
  ...
  finalReadiness(source = readFinalReadiness()) {
    ...
    assert.strictEqual(schemaCount, 137, 'OpenAPI reflete as quatro rotas do IMP-368');
    const openapiSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT, FINAL_READINESS_FILES.openapi))).digest('hex');
    assert.strictEqual(openapiSha, 'c8868afbf0645165da9795f718d91b8fba41bcc2bb8fa111578ec39bed58df0b', 'OpenAPI corresponde ao snapshot governado vigente');
  },
};
