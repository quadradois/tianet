...
const FINAL_READINESS_FILES = {
  ...
  scopeScript: 'scripts/tests/test-imp-303-scope.js',
  workflow: '.github/workflows/quality.yml',
  plan: 'docs/implementation/plans/PLAN-025-frontend-mvp.md',
  backlog: 'docs/implementation/backlogs/PLAN-025-execution-backlog.md',
  discovery: 'docs/audits/discoveries/frontend-mvp-discovery-sdd.md',
...
const contracts = {
  ...
  scaffold(source = readScaffold()) {
    ...
    assertText(source.workflow, 'node scripts/tests/test-imp-303-scope.js', 'CI deve verificar escopo corrente de sanitizacao BFF');
    assertNormalizedText(
      source.workflow,
      '- name: Verify IMP-303 final readiness scope working-directory: . run: node scripts/tests/test-imp-303-scope.js',
  ...
  foundation(source = readFoundation()) {
    ...
    assertText(source.workflow, 'node scripts/tests/test-imp-303-scope.js', 'CI deve verificar escopo corrente IMP-298');
  ...
  openapiClient(source = readOpenapiClient()) {
    ...
    for (const text of ['toHaveLength(30)', 'required).toBe(true)', 'minLength: 1, maxLength: 255', 'toBe(107)', 'toHaveLength(133)']) {
      assertText(source.contractTest, text, `teste contratual cobre ${text}`);
    }
    assertText(source.workflow, 'npm run api:check', 'CI bloqueia drift OpenAPI');
    assertText(source.workflow, 'node scripts/tests/test-imp-303-scope.js', 'CI executa gate corrente IMP-298');
  ...
  dashboard(source = readDashboard()) {
    ...
    assert.doesNotMatch([source.component, source.loader, source.page].join('\n'), /\.reduce\(|parseFloat\(|parseInt\(|Math\.(?:round|floor|ceil)|\bpercentual\b|\b(?:juros|mora|multa|amortiza|saldo|quitacao|renegocia)\w*/i, 'Dashboard nao calcula regra financeira');
    ...
    assertText(source.workflow, 'node scripts/tests/test-imp-303-scope.js', 'CI executa scope corrente IMP-298');
  ...
  comercial(source = readComercial()) {
    ...
    assert.doesNotMatch(source.loader + source.actions + source.bffTest, /Idempotency-Key/, 'Comercial nao inventa Idempotency-Key ausente no OpenAPI certificado');
  ...
  contratos(source = readContratos()) {
    ...
    assert.doesNotMatch(source.loader + source.actions + source.e2eTest, /Idempotency-Key/, 'Contratos nao inventa Idempotency-Key ausente no OpenAPI certificado');
    assertText(source.bffTest + source.contractTest, 'Idempotency-Key', 'testes Contratos provam ausencia de Idempotency-Key');
  ...
  bffErrorSanitization(source = readBffErrorSanitization()) {
    assertText(source.workflow, 'node scripts/tests/test-imp-303-scope.js', 'CI executa scope corrente de sanitizacao BFF');
  ...
  configuracoes(source = readConfiguracoes()) {
    ...
    assertText(source.loader + source.bffTest + source.contractTest, 'sem-idempotency:/credit/configuracoes-financeiras', 'Configuracoes nao inventa Idempotency-Key em criacao');
    assertText(source.loader + source.bffTest + source.contractTest, 'sem-idempotency:/credit/configuracoes-financeiras/snapshots', 'Configuracoes nao inventa Idempotency-Key em snapshot');
    assert.doesNotMatch(source.loader, /Idempotency-Key/, 'loader Configuracoes nao pode enviar Idempotency-Key');
  ...
  iam(source = readIam()) {
    ...
    assertText(source.workflow, 'node scripts/tests/test-imp-303-scope.js', 'CI executa scope corrente IMP-299');
  ...
  automacao(source = readAutomacao()) {
    ...
    assertText(source.workflow, 'node scripts/tests/test-imp-303-scope.js', 'CI executa scope corrente IMP-300');
    ...
    assertText(source.contractTest, '11 operacoes Automacao permitidas', 'contrato Automacao limita 11 operacoes permitidas');
    ...
    ]) assertText(source.loader + source.bffTest + source.contractTest, noIdem, `Automacao nao inventa ${noIdem}`);
  ...
  jornadas(source = readJornadas()) {
    ...
    assertText(source.workflow, 'node scripts/tests/test-imp-303-scope.js', 'CI executa scope corrente IMP-301');
  ...
  certification(source = readCertification()) {
    ...
    assertText(source.workflow, 'node scripts/tests/test-imp-303-scope.js', 'CI executa scope corrente IMP-302');
  ...
  finalReadiness(source = readFinalReadiness()) {
    assertText(source.workflow, 'node scripts/tests/test-imp-303-scope.js', 'CI executa scope final IMP-303');
    ...
    assert.strictEqual(operationCount, 107, 'OpenAPI final preserva 107 operacoes');
    assert.strictEqual(schemaCount, 133, 'OpenAPI final preserva 133 schemas');
    const openapiSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT, FINAL_READINESS_FILES.openapi))).digest('hex');
    assert.strictEqual(openapiSha, '8dadf18eab0dad186044d71e832f72a5850661307d196187f2d0794b9d1d9ec1', 'OpenAPI final preserva SHA governado');
  },
};
...
test('mutacao IMP-289: executar gate de escopo no diretorio frontend e rejeitado', () => {
  ...
  const workflow = scaffold.workflow.replace(
    'working-directory: .\n        run: node scripts/tests/test-imp-303-scope.js',
...
test('mutacao IMP-298: inventar Idempotency-Key no loader e rejeitado', () => {
  const source = readConfiguracoes();
  assert.throws(() => contracts.configuracoes({ ...source, loader: `${source.loader}\nheaders.set("Idempotency-Key", "inventada");` }));
});
...
test('mutacao IMP-293: inventar Idempotency-Key e rejeitado', () => {
