...
const contracts = {
  ...
  hardening(source) {
    ...
    assert.strictEqual(operations, 119, 'snapshot deve conter 119 operacoes');
    assert.strictEqual(Object.keys(snapshot.components.schemas).length, 151, 'snapshot deve conter 151 schemas');
  ...
  harness(source = readHarness()) {
    ...
    assertText(source.contractSmoke, '119', 'contrato deve validar 119 operacoes');
    assertText(source.contractSmoke, '151', 'contrato deve validar 151 schemas');
  ...
  foundation(source = readFoundation()) {
    ...
    assert.deepStrictEqual(
      ...
      ['components/configuracoes-financeiras/configuracoes-actions.client.tsx', 'components/foundation/destructive-dialog-demo.tsx', 'components/openai/openai.client.tsx', 'components/ui/dialog.tsx', 'components/ui/sheet.tsx', 'components/whatsapp/numero-avisos.client.tsx', 'components/whatsapp/whatsapp.client.tsx'],
  ...
  openapiClient(source = readOpenapiClient()) {
    ...
    assertText(
      source.generated,
      '11edf6acc4c134f27357ef3e6e6d9d0e7f68819396c48d36848a87bb16d8b30f',
    ...
    for (const text of ['toBe(119)', 'toHaveLength(151)', 'toHaveLength(65)', 'required).toBe(true)', 'minLength: 1, maxLength: 255']) {
      assertText(source.contractTest, text, `teste contratual cobre ${text}`);
    }
  ...
  finalReadiness(source = readFinalReadiness()) {
    ...
    assert.strictEqual(operationCount, 119, 'OpenAPI reflete as rotas do IMP-353');
    assert.strictEqual(schemaCount, 151, 'OpenAPI reflete as rotas do IMP-353');
    const openapiSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT, FINAL_READINESS_FILES.openapi))).digest('hex');
    assert.strictEqual(openapiSha, '11edf6acc4c134f27357ef3e6e6d9d0e7f68819396c48d36848a87bb16d8b30f', 'OpenAPI corresponde ao snapshot governado vigente');
  },
};
