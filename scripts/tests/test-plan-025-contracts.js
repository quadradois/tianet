...
const contracts = {
  ...
  hardening(source) {
    ...
    assert.strictEqual(Object.keys(snapshot.components.schemas).length, 134, 'snapshot deve conter 134 schemas');
  ...
  harness(source = readHarness()) {
    ...
    assertText(source.contractSmoke, '134', 'contrato deve validar 134 schemas');
  ...
  openapiClient(source = readOpenapiClient()) {
    ...
    for (const text of ['toHaveLength(63)', 'required).toBe(true)', 'minLength: 1, maxLength: 255', 'toBe(107)', 'toHaveLength(134)']) {
      assertText(source.contractTest, text, `teste contratual cobre ${text}`);
    }
  ...
  finalReadiness(source = readFinalReadiness()) {
    ...
    // 107 apos o IMP-332 acrescentar o estorno parcial de Pagamento.
    assert.strictEqual(operationCount, 107, 'OpenAPI inclui o estorno parcial do IMP-332');
    assert.strictEqual(schemaCount, 134, 'OpenAPI inclui o request de estorno parcial do IMP-332');
    const openapiSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT, FINAL_READINESS_FILES.openapi))).digest('hex');
    assert.strictEqual(openapiSha, 'd65e8d85297a0b1dbbe53b67dade22dfe6fb4986267e1f8648b51f865fff1d0b', 'OpenAPI corresponde ao snapshot governado vigente');
  },
};
