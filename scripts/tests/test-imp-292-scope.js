...
const manifestPath = path.join(root, 'docs/audits/evidence/frontend-mvp-imp-292-protected-baseline.json');
const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
const divergences = [];
const sha256 = (absolute) => crypto.createHash('sha256').update(fs.readFileSync(absolute)).digest('hex');

assert.strictEqual(manifest.baselineCount, 192, 'baseline IMP-292 deve conter 192 paths');
assert.strictEqual(manifest.mutableBaselinePaths.length, 15, 'allowlist mutavel IMP-292 deve ter 15 paths');
assert.strictEqual(manifest.protectedBaselineCount, 177, 'IMP-292 deve proteger 177 paths');
assert.strictEqual(manifest.allowedNewPaths.length, 23, 'allowlist nova IMP-292 deve ter 23 paths');
assert.strictEqual(manifest.expectedFinalCount, 215, 'inventario final IMP-292 deve ter 215 paths');
...
assert.strictEqual(sha256(predecessorPath), manifest.predecessor.sha256, 'manifesto verificado do IMP-291 foi reescrito');
...
], 'IMP-292 nao pode criar Route Handler publico de jornada');
...
for (const relative of actualPaths) {
  if (!expectedPaths.has(relative)) divergences.push(`${relative}: caminho extra fora da allowlist do IMP-292`);
}

assert.strictEqual(protectedCount, manifest.protectedBaselineCount, 'contagem protegida IMP-292');
assert.strictEqual(actualPaths.size, manifest.expectedFinalCount, 'contagem do delta final IMP-292');
assert.strictEqual(divergences.length, 0, `escopo protegido do IMP-292 divergiu:\n${divergences.join('\n')}`);
console.log(`IMP-292 scope: predecessor verificado, ${protectedCount} arquivos protegidos e delta exato commitado/worktree (${actualPaths.size} paths), 0 divergencia.`);
