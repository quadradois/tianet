...
const manifestPath = path.join(root, 'docs/audits/evidence/frontend-mvp-imp-290-protected-baseline.json');
const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
const divergences = [];
const sha256 = (absolute) => crypto.createHash('sha256').update(fs.readFileSync(absolute)).digest('hex');

assert.strictEqual(manifest.baselineCount, 150, 'baseline IMP-290 deve conter 150 paths');
assert.strictEqual(manifest.mutableBaselinePaths.length, 15, 'allowlist mutavel IMP-290 deve ter 15 paths');
assert.strictEqual(manifest.protectedBaselineCount, 135, 'IMP-290 deve proteger 135 paths');
assert.strictEqual(manifest.allowedNewPaths.length, 19, 'allowlist nova IMP-290 deve ter 19 paths');
assert.strictEqual(manifest.expectedFinalCount, 169, 'inventario final IMP-290 deve ter 169 paths');
...
assert.strictEqual(sha256(predecessorPath), manifest.predecessor.sha256, 'manifesto verificado do IMP-289 foi reescrito');
...
], 'IMP-290 nao pode criar Route Handler');
...
for (const relative of actualPaths) {
  if (!expectedPaths.has(relative)) divergences.push(`${relative}: caminho extra fora da allowlist do IMP-290`);
}

assert.strictEqual(protectedCount, manifest.protectedBaselineCount, 'contagem protegida IMP-290');
assert.strictEqual(actualPaths.size, manifest.expectedFinalCount, 'contagem do delta final IMP-290');
assert.strictEqual(divergences.length, 0, `escopo protegido do IMP-290 divergiu:\n${divergences.join('\n')}`);
console.log(`IMP-290 scope: predecessor verificado, ${protectedCount} arquivos protegidos e delta exato commitado/worktree (${actualPaths.size} paths), 0 divergencia.`);
