...
const manifestPath = path.join(root, 'docs/audits/evidence/frontend-mvp-imp-296-protected-baseline.json');
const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
const divergences = [];
const sha256 = (absolute) => crypto.createHash('sha256').update(fs.readFileSync(absolute)).digest('hex');

assert.strictEqual(manifest.baselineCount, 280, 'baseline IMP-296 deve conter 280 paths');
assert.strictEqual(manifest.mutableBaselinePaths.length, 10, 'allowlist mutavel IMP-296 deve ter 10 paths');
assert.strictEqual(manifest.protectedBaselineCount, 270, 'IMP-296 deve proteger 270 paths');
assert.strictEqual(manifest.allowedNewPaths.length, 21, 'allowlist nova IMP-296 deve ter 21 paths');
assert.strictEqual(manifest.expectedFinalCount, 301, 'inventario final IMP-296 deve ter 301 paths');
...
assert.strictEqual(sha256(predecessorPath), manifest.predecessor.sha256, 'manifesto verificado do IMP-295 foi reescrito');
...
], 'IMP-296 nao pode criar Route Handler publico de jornada');
...
for (const relative of actualPaths) {
  if (!expectedPaths.has(relative)) divergences.push(`${relative}: caminho extra fora da allowlist do IMP-296`);
}

assert.strictEqual(protectedCount, manifest.protectedBaselineCount, 'contagem protegida IMP-296');
assert.strictEqual(actualPaths.size, manifest.expectedFinalCount, 'contagem do delta final IMP-296');
assert.strictEqual(divergences.length, 0, `escopo protegido do IMP-296 divergiu:\n${divergences.join('\n')}`);
console.log(`IMP-296 scope: predecessor verificado, ${protectedCount} arquivos protegidos e delta exato commitado/worktree (${actualPaths.size} paths), 0 divergencia.`);
