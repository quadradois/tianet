...
**GATE-E4** — IMP-381..384. Pré-requisitos operacionais: **cumpridos em 2026-09-22.** Os três segredos do incidente de 2026-09-18 rotacionados (`PLATFORM_ADMIN_BOOTSTRAP_SECRET_HASH` na recuperação de credencial; `TIANET_AGENT_INTERNAL_SECRET` e `POSTGRES_PASSWORD` com os seis containers recriados e zero erro de autenticação); reboot do kernel feito, com a stack voltando sozinha graças à política de reinício do PR #105; runbook da ponte em `docs/operations/runbook-ponte-do-agent.md`. Condição: envelope autenticado em produção; conversa real observada; handoff.
