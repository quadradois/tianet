...
2. Como o identificador é seu, ele **correlaciona** requisição e resposta — e é
   ...
   ⚠️ **Corrigido em 2026-09-02.** Até então eram três defeitos vivos:
   `EvolutionWhatsAppNotificationChannel` classificava **todo timeout e todo erro
   de transporte** (incluindo resets *depois* do envio, como `ReadError`,
   `CloseError` e `RemoteProtocolError`) e **toda resposta 5xx** como `FALHA_TEMPORARIA`, e
   deixa **`DecodingError` escapar** para o Scheduler, que trata qualquer
   ...
   **viola a ADR-009**, cuja tabela põe `5xx` e "timeout/reset após transmitir
   ...
   é resultado desconhecido.

   Hoje o adapter aplica exatamente essa allowlist, e os testes cobrem os três
   caminhos anteriores ao envio e os sete que não provam nada. **O que continua
   aberto é a premissa**: ninguém mediu se o Evolution deduplica pelo `id` — e é
   por isso que cada resultado desconhecido vira conciliação humana. Ver
...
1. **`DELETE /instance/delete/:id` num ID inexistente retorna `500`, não `404`.** Trate como "não existe" na prática, não como erro de infraestrutura. **Ampliado em 2026-09-02 (IMP-368), por leitura ao vivo:** `GET /instance/info/:id` faz o mesmo — `500` com `{"error":"record not found"}` no corpo, e `500` com `{"error":"invalid UUID format: ..."}` quando o id nem é UUID. **O status mente; o corpo não.** Distinga ausência pelo texto do corpo, e não pelo `500`: tratar todo `500` como "já não existe" faria você apagar o registro local afirmando uma exclusão que talvez não tenha acontecido — e a instância órfã ficaria sem nada apontando para ela.
